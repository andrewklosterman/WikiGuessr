...
// These functions run after guessing, one for each option chosen. If the two articles have equal views or length and the player chose that option, the player neither gains a point nor loses a life.
function choseNW() {
	if (articleOneLength > articleTwoLength) {score++}
...
function choseNE() {
	if (articleTwoLength > articleOneLength) {score++}
...
function choseSW() {
	if (articleOneViews > articleTwoViews) {score++}
...
function choseSE() {
	if (articleTwoViews > articleOneViews) {score++}
